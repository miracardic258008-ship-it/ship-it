...
function startGPS(){
  ...
  navigator.geolocation.getCurrentPosition(onPos,()=>{},{enableHighAccuracy:false,timeout:5000,maximumAge:60000});
  navigator.geolocation.watchPosition(onPos,()=>{},{enableHighAccuracy:true,maximumAge:3000,timeout:30000});
}
...
function onPos(p){
  ...
  // Hız göstergesi
  const spdEl=document.getElementById('spd-val')||document.getElementById('spd-v')||document.getElementById('spd-ring-v');
  if(spdEl) spdEl.textContent=curSpeed;
...
async function startNav(name,lat,lng){
  ...
  closeRoutePicker();
  showEl('reroute','↻ Hesaplanıyor...');
  // Navigasyon başlayınca sidebar/topbar/arama gizle
  document.getElementById('sidebar').style.display='none';
  document.getElementById('topbar').style.display='none';
  hideEl('search-wrap');hideEl('hud');
...
function _beginNav(name,lat,lng,announce){
  ...
  document.getElementById('hud-arrow').textContent=mi(fs?.type,fs?.mod);
  document.getElementById('hud-txt').textContent=fs?.text||'';
...
function stopNav(){
  ...
  _restoreUI();
  // Kamera butonunu kaldır
  const cb=document.getElementById('cam-btn');if(cb)cb.remove();
...
function updateNav(lat,lng){
  ...
  document.getElementById('hud-txt').textContent=step.text;
  ...
  if(d<20&&stepIdx<steps.length-1){
    ...
    document.getElementById('hud-arrow').textContent=mi(s2.type,s2.mod);
    return;
  }
...
function hideEl(id){
  ...
  if(['search-wrap','nav-bar','hud'].includes(id))el.style.display='none';
}
function toggleLayer(){
  document.getElementById('layer-panel').classList.toggle('on');
  document.getElementById('settings-panel').classList.remove('on');
}
function closeLayer(){document.getElementById('layer-panel').classList.remove('on');}
function toggleSettings(){
  document.getElementById('settings-panel').classList.toggle('on');
  document.getElementById('layer-panel').classList.remove('on');
}
function closeSettings(){document.getElementById('settings-panel').classList.remove('on');}
function openMenu(){
  document.getElementById('menu-panel').classList.add('on');
  document.getElementById('menu-overlay').classList.add('on');
}
function closeMenu(){
  document.getElementById('menu-panel').classList.remove('on');
